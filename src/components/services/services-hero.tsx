  ...
  return (
    <section className="relative -top-[85.75px] flex h-dvh items-center justify-center pt-[85.75px]">
      ...
      <div className="relative z-10 mx-auto flex w-fit max-w-7xl flex-col items-center px-10 py-8 sm:flex-row sm:gap-22 lg:px-16 lg:py-12">
        <div className="text-pwr-primary-foreground mt-8 space-y-6 sm:mt-0">
          ...
          <ul className="space-y-4 sm:space-y-2">
            {serviceMainPoints.map((point, ind) => (
              <li key={`${point}-${ind}`} className="flex text-base font-medium md:text-lg">
        ...
        <div className="bg-pwr-primary relative top-[65px] w-full rounded-md px-10 py-8 shadow-[0_0_50px_rgba(0,0,0,0.35)] sm:top-[90px] sm:w-fit lg:px-16 lg:py-12">
          <ContactUs isPrimaryForeground classNames={{ form: "hero-contact:grid-cols-2" }} />
        </div>
      </div>
    </section>
  );
